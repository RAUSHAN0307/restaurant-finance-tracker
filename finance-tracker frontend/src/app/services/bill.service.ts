...
export class BillService {
  ...
  getMenuItems(): Observable<MenuItemOption[]> {
    return this.http
      .get<any[]>(`${this.BASE}/menu/user/${this.getApiUserId()}`)
      .pipe(
        map((list) => {
  ...
  getVouchers(): Observable<VoucherOption[]> {
    return this.http
      .get<any[]>(`${this.BASE}/vouchers/user/${this.getApiUserId()}`)
      .pipe(
        map((list) =>
          list.map((v) => ({
  ...
  getBillItems(billId: number): Observable<any[]> {
    return this.http.get<any[]>(`${this.BASE}/bill-items/bill/${billId}`);
  }
  ...
  deleteBill(billId: number): Observable<any> {
    return this.http.delete(`${this.BASE}/bills/${billId}`, {
      responseType: "text",
    });
  }
}
