...
export class EmployeesService {
  private readonly BASE = "http://localhost:3000/employee";
  // private readonly BASE        = 'http://192.168.1.39:3000/employee';
  private readonly SALARY_BASE = "http://localhost:3000/salary";
  // private readonly SALARY_BASE = "http://192.168.1.39:3000/salary";
  private readonly AUTH_BASE = "http://localhost:3000/api";
  ...
  private mapEmployee(e: any): Employee {
    ...
    return {
      ...
      joining: this.parseDate(
        e.joinDate ?? e.joining ?? e.joinedDate ?? e.joiningDate,
      ),
      role: displayRole,
      dept: e.dept ?? "",
      salary: e.salary ?? 0,
  ...
  private rollbackUserRegistration(userId: number): void {
    console.warn(
      "[AUTH 192.168.1.39:3000] ROLLBACK — deleting userId:",
      userId,
    );
    this.http
      .delete(`${this.AUTH_BASE}/users/delete/${userId}`, {
        responseType: "text",
      })
      .subscribe({
  ...
  getAll(): Observable<Employee[]> {
    ...
    return this.http.get<any[]>(`${this.BASE}/user/${apiUserId}`).pipe(
    ...
    if (needsCredentials && employee.username && employee.password) {
      return this.registerUserCredentials(
        ...
        switchMap((userRes) => {
          ...
          console.log(
            "[EMP] INSERT — apiUserId:",
            apiUserId,
            "— payload:",
            body,
          );

          return this.http.post<any>(`${this.BASE}/${apiUserId}`, body).pipe(
      ...
      const body = this.toApiPayload(employee);
      console.log(
        "[EMP] CREATE (no credentials) — apiUserId:",
        apiUserId,
        "— payload:",
        body,
      );
  ...
  delete(id: number): Observable<string> {
    ...
    return this.http
      .delete(`${this.BASE}/${id}`, { responseType: "text" })
      .pipe(tap((msg) => console.log("[EMP] DELETE response:", msg)));
  }
  ...
  paySalary(
    ...
    return this.http.post<any>(`${this.SALARY_BASE}/${employeeId}`, body).pipe(
  ...
  getSalariesByEmployee(employeeId: number): Observable<SalaryRecord[]> {
    return this.http
      .get<any[]>(`${this.SALARY_BASE}/${employeeId}`)
      .pipe(
