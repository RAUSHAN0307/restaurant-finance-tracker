...
export class AuthService {
  ...
  registerOwner(payload: RegisterPayload): Observable<LoginResponse> {
    return this.http
      .post<LoginResponse>(`${this.BASE}/users/register-owner`, payload)
  ...
  login(
    ...
    return this.http
      .post<LoginResponse>(`${this.BASE}/auth/v1/sign-in`, {
        username,
        password,
      })
      .pipe(
  ...
  getUserById(userId: number): Observable<AuthUser> {
    return this.http.get<AuthUser>(`${this.BASE}/users/get/${userId}`);
  }
  ...
  updateUser(
    ...
    return this.http
      .put<AuthUser>(`${this.BASE}/users/update/${userId}`, changes)
      .pipe(
        tap((updated) => {
  ...
  deleteUser(userId: number): Observable<string> {
    return this.http.delete(`${this.BASE}/users/delete/${userId}`, {
      responseType: "text",
    });
  }
