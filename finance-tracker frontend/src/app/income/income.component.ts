...
export class IncomeComponent implements OnInit {
  ...
  private mapBill(b: Bill): IncomeRecord {
    ...
    const isoDate =
      parts.length === 3 ? `${parts[2]}-${parts[1]}-${parts[0]}` : b.billDate;
  ...
  applyFilters(): void {
    ...
    this.filtered = this.ALL.filter((r) => {
      ...
      if (this.fType) {
        ...
        const normalizedFilter = this.fType
          .toUpperCase()
          .replace(/\s+/g, " ")
          .trim();
      ...
      // Payment filter
      if (this.fPayment && r.payment !== this.fPayment.toUpperCase())
        return false;
      ...
      if (search) {
        const hay =
          `${r.displayDate} ${r.date} ${r.type} ${r.payment} ${r.gross} ${r.net} ${r.phoneNo} ${r.billId}`.toLowerCase();
  ...
  payBadgeClass(pay: string): string {
    return this.PAY_CLS[pay] || "";
  }
