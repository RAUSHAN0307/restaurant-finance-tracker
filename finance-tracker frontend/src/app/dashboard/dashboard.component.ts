...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild("incomeExpenseCanvas")
  incomeExpenseCanvas!: ElementRef<HTMLCanvasElement>;
  ...
  ngOnDestroy(): void {
    [this.incomeExpenseChart, this.expensePieChart, this.profitChart].forEach(
      (c) => {
        if (c) c.destroy();
      },
    );
  ...
  loadHeaderProfile(): void {
    try {
      ...
      if (authUser)
        this.headerRoleLabel = (authUser.role || "admin").toUpperCase();
      const raw = localStorage.getItem("ftProfile");
      if (raw) {
        const p = JSON.parse(raw);
        if (p.role) this.headerRoleLabel = p.role.toUpperCase();
      }
    } catch (e) {
      this.headerRoleLabel = "ADMIN";
    }
  }
  ...
  private getDateRange(): { startDate: string; endDate: string } {
    ...
    if (this.rangeStart && this.rangeEnd)
      return {
        startDate: this.toApiDate(this.rangeStart),
        endDate: this.toApiDate(this.rangeEnd),
      };
    ...
    switch (this.currentPeriod) {
      ...
      case "prev-year": {
        ...
        return {
          startDate: this.toApiDate(prevStart),
          endDate: this.toApiDate(prevEnd),
        };
      }
      default:
        return { startDate: today, endDate: today };
      ...
      next: (summary) => {
        ...
        this.http.get<ChartApiResponse>(url2).subscribe({
          ...
          error: (err) => {
            ...
            this.showToast(
              "Chart data unavailable, showing totals only",
              "error",
            );
          },
        });
  ...
  setSession(session: string): void {
    ...
    if (session !== "all-sessions") {
      this.showToast(
        "Session breakdown is not available from the API.",
        "error",
      );
    }
    if (this.analyticsData) {
      ...
      this.incomeExpenseSubtitle =
        (this.PERIOD_LABELS[this.currentPeriod] || "Custom") +
        " · " +
  ...
  resetZoom(): void {
    if (this.profitChart) {
      try {
        this.profitChart.resetZoom();
      } catch (e) {}
    }
  }

  isPeriodActive(p: string): boolean {
    return this.currentPeriod === p;
  }
  isSessionActive(s: string): boolean {
    return this.currentSession === s;
  }
  ...
  buildCharts(): void {
    ...
    try {
      const zp =
        (window as any).ChartZoom || (window as any)["chartjs-plugin-zoom"];
      if (zp) Chart.register(zp);
    } catch (e) {}

    // ── Bar: Income vs Expense ──
    this.incomeExpenseChart = new Chart(
      this.incomeExpenseCanvas.nativeElement,
      {
        type: "bar",
        data: {
          labels: [],
  ...
  updateAllCharts(
    ...
    if (!this.incomeExpenseChart || !this.expensePieChart || !this.profitChart)
      return;
  ...
  private updatePieChart(summary: AnalyticsResponse): void {
    ...
    if (this.currentCategory !== "all") {
      ...
      const allowed = (CAT_MAP[this.currentCategory] || []).map((s) =>
        s.toLowerCase(),
      );
      breakdown = breakdown.filter((b) =>
        allowed.includes(b.category.toLowerCase()),
      );
    }
    ...
    this.expensePieChart.data.datasets[0].backgroundColor =
      this.PIE_COLORS.slice(0, pieLabels.length);
  ...
  private updateSubtitleTags(): void {
    ...
    const sLabel =
      this.SESSION_LABELS[this.currentSession] || this.currentSession;
    const sel = document.getElementById("categoryFilter") as HTMLSelectElement;
    const cLabel =
      this.currentCategory === "all"
        ? "All Categories"
    ...
    this.activeSessionTag =
      this.currentSession !== "all-sessions" ? sLabel : "";
    this.activeCategoryTag = this.currentCategory !== "all" ? cLabel : "";
    this.activeDateTag = this.currentDateRange
