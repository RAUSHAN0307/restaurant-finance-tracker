...
export class ExpenseComponent implements OnInit {
  ...
  private readonly BASE_URL = "http://localhost:3000/expenses";

  private get FETCH_URL() {
    return `${this.BASE_URL}/user/${this.getApiUserId()}`;
  }
  private get CREATE_URL() {
    return `${this.BASE_URL}/${this.getApiUserId()}`;
  }
  ...
  fetchExpenses(): void {
    ...
    this.http.get<ApiExpense[]>(this.FETCH_URL).subscribe({
      ...
      error: (err) => {
        ...
        this.loadError =
          "Failed to load expenses. Check the server and try again.";
        this.isLoading = false;
        this.applyFilters();
      },
    });
  ...
  private createExpense(): void {
    ...
    this.http
      .post<ApiExpense>(this.CREATE_URL, payload, {
      ...
      .subscribe({
        next: (saved) => {
          ...
          this.isSaving = false;
          this.showToast(
            `"${saved.expenseName}" added successfully`,
            "success",
          );
          this.closeModal();
          this.applyFilters();
  ...
  private updateExpense(): void {
    ...
    this.http
      .put<ApiExpense>(this.editUrl(this.editingId!), payload, {
        headers: this.JSON_HEADERS,
      })
      .subscribe({
        next: (updated) => {
          ...
          this.isSaving = false;
          this.showToast(
            `"${updated.expenseName}" updated successfully`,
            "success",
          );
          this.closeModal();
          this.applyFilters();
  ...
  doDelete(): void {
    ...
    this.http
      .delete(this.deleteUrl(idToDelete), { responseType: "text" })
      .subscribe({
        next: () => {
