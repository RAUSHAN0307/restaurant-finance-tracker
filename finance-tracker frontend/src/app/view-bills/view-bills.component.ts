...
export class ViewBillsComponent implements OnInit, OnDestroy {
  ...
  private get API_URL(): string {
    return `http://localhost:3000/bills/user/${this.getApiUserId()}`;
  }
  ...
  private mapApiBill(b: ApiBill): Bill {
    ...
    const taxPercent =
      b.totalAmt > 0 ? Math.round((b.taxAmt / b.totalAmt) * 100) : 0;
  ...
  calcSavings(b: Bill): number {
    return (b.voucherAmt || 0) + (b.discount || 0);
  }
  ...
  applyFilters(): void {
    ...
    this.filtered = this.allBills.filter((b) => {
      ...
      const matchPayment =
        !this.filterPayment || b.payment === this.filterPayment;
  ...
  downloadPdf(bill: Bill, event?: Event): void {
    ...
    doc.text(`Rs ${bill.subtotal.toFixed(2)}`, 192, finalY + 10, {
      align: "right",
    });
    doc.text("Tax:", 124, finalY + 20);
    doc.text(`Rs ${bill.taxAmt.toFixed(2)}`, 192, finalY + 20, {
      align: "right",
    });
    ...
    if (bill.voucherAmt > 0) {
      ...
      doc.text(`-Rs ${bill.voucherAmt.toFixed(2)}`, 192, lineY, {
        align: "right",
      });
      doc.setTextColor(100, 100, 100);
      lineY += 10;
